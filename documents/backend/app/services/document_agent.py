...
class DocumentAgentService:
    ...
    async def process_csv_upload(
        ...
        if manual_data.get("signatory_signature"):
            ...
            print(f"🔍 Found global signature in context: {global_sig_b64[:50]}..." if global_sig_b64 else "❌ No global signature found")
        else:
            print(f"❌ No manual_data or signature in context. Context keys: {list(ctx.keys())}")
        ...
        for idx, row_data in enumerate(csv_data, 1):
            ...
            if not sig_b64 and global_sig_b64:
                ...
                print(f"✅ Row {idx}: Applied global signature")
            if sig_b64:
                row_data["signatory_signature"] = sig_b64
                # FORCE skip text fields when signature is present (override CSV values)
                row_data["signatory_name"] = "skip"
                row_data["signatory_designation"] = "skip"
                print(f"✅ Row {idx}: Signature added to row_data (signatory_name and designation FORCED to 'skip')")
            else:
                print(f"⚠️ Row {idx}: No signature available (global or row-specific)")
            # Generate document content
    ...
    async def generate_preview(self, session_id: str) -> Dict[str, Any]:
        ...
        print(f"🔍 DEBUG template_path='{template_path}', exists={template_path.exists()}")
        print(f"🔍 DEBUG manual_data keys: {list(manual_data.keys())}")
        print(f"🔍 DEBUG has signature: {bool(manual_data.get('signatory_signature'))}")
    ...
    async def generate_documents(
        ...
        template = await self._get_template(conversation.selected_template_id)
        template_path = self.templates_dir / self.template_file_map.get(template.name.lower(), "")
...
"""
        for field_name, value in data.items():
            if field_name == "signatory_signature" and value and value != "skip":
                # Handle signature specially
                signature_src = value if value.startswith("data:image") else f"data:image/png;base64,{value}"
                html += f'<div class="signature"><div class="label">Signature:</div><img alt="Signature" src="{signature_src}" /></div>\n'
            elif value != "skip":
