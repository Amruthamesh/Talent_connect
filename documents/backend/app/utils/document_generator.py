...
class DocumentGenerator:
    ...
    def _replace_placeholders(cls, template_text: str, data: Dict[str, Any]) -> str:
        ...
        for key, value in data.items():
            ...
            if key == "employee_name" or key == "candidate_name":
                ...
                # Don't replace if signature will be used (skip means signature replaces it)
                if value_str != "skip":
    ...
    def generate_preview_html(
        ...
        print(f"🔍 DEBUG generate_preview_html: has signature data = {bool(signature_b64)}, is skip = {signature_b64 == 'skip'}")
        if signature_b64 and signature_b64 != "skip":
...
<div style="margin: 20px 0;">
    <img alt="Signature" src="{signature_src}" style="max-width: 300px; height: auto; display: block;" />
</div>'''
            print(f"✅ DEBUG Signature HTML created, length: {len(signature_html)}")
...
"""
        ...
        if signature_html:
            print(f"🔍 DEBUG Attempting to inject signature into HTML")
            # Replace both title-case and uppercase placeholders inside the HTML-formatted content
            replaced = False
            if "[Signatory Name]" in html:
                html = html.replace("[Signatory Name]", signature_html)
                replaced = True
                print(f"✅ DEBUG Replaced [Signatory Name] with signature")
            if "[SIGNATORY NAME]" in html:
                html = html.replace("[SIGNATORY NAME]", signature_html)
                replaced = True
                print(f"✅ DEBUG Replaced [SIGNATORY NAME] with signature")
            # Remove designation placeholders (signature image contains both name and designation)
            html = html.replace("[Signatory Designation]", "")
            html = html.replace("[SIGNATORY DESIGNATION]", "")
            
            # Fallback: if no placeholders were present, append signature near the end
            if not replaced:
                print(f"⚠️ DEBUG No signatory placeholders found, using fallback injection")
                # Try to insert before closing content div
                if "</div>\n</body>" in html:
                    html = html.replace("</div>\n</body>", f"{signature_html}</div>\n</body>")
                    print(f"✅ DEBUG Injected signature before </div>\\n</body>")
                elif "</div>" in html:
                    # Find last occurrence of content div closing
                    parts = html.rsplit("</div>", 2)
                    if len(parts) > 1:
                        html = parts[0] + signature_html + "</div>" + parts[1]
                        print(f"✅ DEBUG Injected signature using rsplit method")
        return html
